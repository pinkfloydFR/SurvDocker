...
3. Éditer `survdocker/config/survdocker.yml` uniquement pour les réglages métier (scan, filtres, criticité).
...
Le fichier central contient déjà des commentaires pour expliquer chaque paramètre métier.
...
- `survdocker/system/alloy.alloy` : configuration de collecte Alloy, générée à partir du fichier central.
- `survdocker/system/loki-config.yml` : configuration Loki, générée à partir du fichier central.
- `docker-compose.yml` : orchestration des services.

Important :
Les paramètres d’infrastructure (réseau, Traefik, URL Loki, endpoints/secrets Telegram) sont pilotés via `.env` et `docker-compose.yml`, pas via `survdocker.yml`.
