...
def notifications_configured(settings) -> bool:
    telegram_ready = bool(settings.telegram.enabled and settings.telegram.bot_token and settings.telegram.chat_id)
    apprise_ready = bool(settings.apprise.enabled and settings.apprise.url)
...
def notify(settings, title: str, text: str) -> list[NotifyResult]:
    ...
    if settings.apprise.enabled and settings.apprise.url:
